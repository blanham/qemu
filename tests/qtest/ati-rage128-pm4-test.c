...
static void load_linux_cce_microcode(Rage128PM4Test *test,
                                      const char *path,
                                      uint32_t selected[4])
{
    ...
    g_assert_true(g_file_get_contents(path, &firmware, &length, &error));
    g_assert_no_error(error);
    g_assert_cmpuint(length, ==, 256U * 8U);

    mmio_write(test, R128_PM4_MICROCODE_ADDR, 0);
    for (unsigned int i = 0; i < 256; i++) {
        uint32_t high = ldl_be_p(firmware + i * 8U);
        uint32_t low = ldl_be_p(firmware + i * 8U + 4U);

        mmio_write(test, R128_PM4_MICROCODE_DATAH, high);
        mmio_write(test, R128_PM4_MICROCODE_DATAL, low);
        if (i == 0) {
            selected[0] = high;
            selected[1] = low;
        } else if (i == 37) {
    ...
    mmio_write(test, R128_PM4_MICROCODE_RADDR, 0);
...
static void linux_engine_reset(Rage128PM4Test *test)
{
    uint32_t clock_index = mmio_read(test, R128_CLOCK_CNTL_INDEX);
    uint32_t gen_reset = mmio_read(test, R128_GEN_RESET_CNTL);
    uint32_t mclk;

    mmio_write(test, R128_PC_NGUI_CTLSTAT,
               mmio_read(test, R128_PC_NGUI_CTLSTAT) |
               R128_PC_FLUSH_ALL);
    g_assert_cmphex(mmio_read(test, R128_PC_NGUI_CTLSTAT) &
                    R128_PC_BUSY, ==, 0);

    mmio_write(test, R128_CLOCK_CNTL_INDEX, R128_MCLK_CNTL);
    mclk = mmio_read(test, R128_CLOCK_CNTL_DATA);
    mmio_write(test, R128_CLOCK_CNTL_INDEX,
               R128_MCLK_CNTL | R128_PLL_WR_EN);
    mmio_write(test, R128_CLOCK_CNTL_DATA,
               mclk | R128_FORCE_GCP | R128_FORCE_PIPE3D_CP);

    mmio_write(test, R128_GEN_RESET_CNTL,
               gen_reset | R128_SOFT_RESET_GUI);
    (void)mmio_read(test, R128_GEN_RESET_CNTL);
    mmio_write(test, R128_GEN_RESET_CNTL,
               gen_reset & ~R128_SOFT_RESET_GUI);
    (void)mmio_read(test, R128_GEN_RESET_CNTL);
    ...
    mmio_write(test, R128_CLOCK_CNTL_INDEX,
               R128_MCLK_CNTL | R128_PLL_WR_EN);
    mmio_write(test, R128_CLOCK_CNTL_DATA, mclk);
    mmio_write(test, R128_CLOCK_CNTL_INDEX, clock_index);
    mmio_write(test, R128_GEN_RESET_CNTL, gen_reset);
...
static void test_pm4_linux_cce_lifecycle(void)
{
    const char *firmware_path = g_getenv("RAGE128_CCE_FIRMWARE");
    Rage128PM4Test *test;
    RingBuilder ring = { 0 };
    uint32_t selected[4] = { 0 };
    uint32_t status;
    uint32_t read_pointer;

    if (!firmware_path || !*firmware_path) {
        g_test_skip("RAGE128_CCE_FIRMWARE is not set");
        return;
    }
    ...
    /* r128_cce_init_ring_buffer(): ring, pointers, watermarks, and BM. */
    mmio_write(test, R128_PM4_BUFFER_OFFSET, GART_VIRT);
    mmio_write(test, R128_PM4_BUFFER_DL_WPTR, 0);
    mmio_write(test, R128_PM4_BUFFER_DL_RPTR, 0);
    mmio_write(test, R128_PM4_BUFFER_WM_CNTL, 0x02020204U);
    (void)mmio_read(test, R128_PM4_BUFFER_ADDR);
    mmio_write(test, R128_BUS_CNTL,
               mmio_read(test, R128_BUS_CNTL) & ~R128_BUS_MASTER_DIS);
    mmio_write(test, R128_PM4_BUFFER_DL_RPTR_ADDR,
               GART_VIRT + 0x2000U);

    /* r128_cce_load_microcode() with the shipped Linux firmware. */
    load_linux_cce_microcode(test, firmware_path, selected);

    /* r128_do_engine_reset() must retain ring/GART and uploaded firmware. */
    linux_engine_reset(test);
    g_assert_cmphex(mmio_read(test, R128_PCI_GART_PAGE), ==, GART_PHYS);
    g_assert_cmphex(mmio_read(test, R128_PM4_BUFFER_OFFSET), ==, GART_VIRT);
    g_assert_cmphex(mmio_read(test, R128_PM4_BUFFER_WM_CNTL), ==,
                    0x02020204U);
    ...
    /* r128_do_cce_start(), a real ring dispatch, flush, and idle test. */
    mmio_write(test, R128_PM4_BUFFER_CNTL,
               R128_PM4_192BM | R128_PM4_BUFFER_CNTL_NOUPDATE | 9U);
    (void)mmio_read(test, R128_PM4_BUFFER_ADDR);
    mmio_write(test, R128_PM4_MICRO_CNTL, R128_PM4_MICRO_FREERUN);

    ring_packet0_one(&ring, R128_GUI_SCRATCH_REG0, 0x4c434345U);
    ring_emit(&ring, R128_PM4_PACKET2);
    upload_ring(test, &ring);
    mmio_write(test, R128_PM4_BUFFER_DL_WPTR, ring.count);
    ...
    g_assert_cmpuint(mmio_read(test, R128_PM4_BUFFER_DL_WPTR), ==,
                     ring.count);
    status = mmio_read(test, R128_PM4_STAT);
    g_assert_cmpuint(status & R128_PM4_FIFO_COUNT_MASK, >=, 192U);
    g_assert_cmphex(status & (R128_PM4_BUSY | R128_PM4_GUI_ACTIVE), ==, 0);
    mmio_write(test, R128_PC_NGUI_CTLSTAT,
               mmio_read(test, R128_PC_NGUI_CTLSTAT) |
               R128_PC_FLUSH_ALL);
    g_assert_cmphex(mmio_read(test, R128_PC_NGUI_CTLSTAT) &
    ...
    /* r128_do_cce_stop(), followed by the driver's final engine reset. */
    mmio_write(test, R128_PM4_MICRO_CNTL, 0);
    mmio_write(test, R128_PM4_BUFFER_CNTL,
               R128_PM4_BUFFER_CNTL_NOUPDATE);
    linux_engine_reset(test);
    g_assert_cmphex(mmio_read(test, R128_PM4_MICRO_CNTL), ==, 0);
    g_assert_cmpuint(mmio_read(test, R128_PM4_BUFFER_DL_RPTR), ==, 0);
    g_assert_cmpuint(mmio_read(test, R128_PM4_BUFFER_DL_WPTR), ==, 0);
    mmio_write(test, R128_PM4_MICROCODE_RADDR, 0);
    g_assert_cmphex(mmio_read(test, R128_PM4_MICROCODE_DATAH), ==,
...
static void test_pm4_dri1_state_swap_flip(void)
{
    ...
    /*
     * Xorg establishes forward X/Y directions before the kernel emits swap
     * packets.  Replay that persistent engine precondition instead of
     * relying on reset state.
     */
    mmio_write(test, R128_DP_CNTL, 3);

    /*
     * Replay r128_emit_state(): core, context, paired setup, masks, window,
     * and two clip rectangles combined by the auxiliary-clip OR mode.
     */
...
int main(int argc, char **argv)
{
    ...
    g_test_add_func("/ati/rage128/pm4-linux-cce-lifecycle",
                    test_pm4_linux_cce_lifecycle);
