...
#define R128_GMC_DST_16BPP                  0x00000400
...
static void test_rage128_color_compare(void)
{
    ...
    static const uint8_t source[8] = {
        0x10, 0x22, 0x30, 0x22, 0x40, 0x50, 0x22, 0x60,
    };
    static const uint8_t destination[8] = {
        0x55, 0x11, 0x55, 0x33, 0x55, 0x44, 0x55, 0x66,
    };

    memset(fb, 0xaa, sizeof(fb));
    memcpy(&fb[0], source, sizeof(source));
    memcpy(&fb[2 * 32], destination, sizeof(destination));
    memcpy(&fb[3 * 32], destination, sizeof(destination));
    memset(&fb[4 * 32], 0xcc, 8);
    qpci_memwrite(test->dev, test->framebuffer, 0, fb, sizeof(fb));
    rage128_set_surface(test, 4); /* 32-byte 8-bpp stride */

    /* Source-keyed screen copy, matching the historical XAA setup. */
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_BRUSH_NONE |
                  R128_GMC_DST_8BPP | R128_GMC_SRC_DATATYPE_COLOR |
                  R128_ROP3_SRCCOPY | R128_DP_SRC_SOURCE_MEMORY);
    rage128_write(test, R128_CLR_CMP_CLR_SRC, 0x22);
    rage128_write(test, R128_CLR_CMP_CLR_DST, 0x55);
    ...
    g_assert_cmphex(rage128_read(test, R128_CLR_CMP_CLR_SRC), ==, 0x22);
    g_assert_cmphex(rage128_read(test, R128_CLR_CMP_CLR_DST), ==, 0x55);
    g_assert_cmphex(rage128_read(test, R128_CLR_CMP_MASK), ==, 0xff);
    ...
    qpci_memread(test->dev, test->framebuffer, 0, fb, sizeof(fb));
    for (unsigned int x = 0; x < 8; x++) {
        g_assert_cmpuint(fb[32 + x], ==,
                         source[x] == 0x22 ? 0xaa : source[x]);
    }

    /* The master-control disable bit clears stale comparison state. */
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
    ...
    g_assert_cmphex(rage128_read(test, R128_CLR_CMP_CNTL), ==, 0);

    /* Destination equality gates an ordinary solid rectangle. */
    rage128_write(test, R128_CLR_CMP_CLR_DST, 0x55);
    rage128_write(test, R128_CLR_CMP_MASK, 0xff);
    rage128_write(test, R128_CLR_CMP_CNTL, R128_SRC_CMP_EQ_COLOR);
    rage128_write(test, R128_DP_BRUSH_FRGD_CLR, 0xe7);
    rage128_write(test, R128_DST_X, 0);
    ...
    for (unsigned int x = 0; x < 8; x++) {
        g_assert_cmpuint(fb[2 * 32 + x], ==,
                         destination[x] == 0x55 ? 0xe7 : destination[x]);
    }

    /* Destination comparison also gates register-programmed lines. */
    rage128_write(test, R128_DP_CNTL_XDIR_YDIR_YMAJOR,
                  R128_LINE_X_LEFT_TO_RIGHT |
                  R128_LINE_Y_TOP_TO_BOTTOM);
    rage128_write(test, R128_DP_BRUSH_FRGD_CLR, 0xd1);
    rage128_write(test, R128_DST_Y_X, 3U << 16);
    rage128_write(test, R128_DST_BRES_ERR, UINT32_MAX);
    rage128_write(test, R128_DST_BRES_INC, 0);
    rage128_write(test, R128_DST_BRES_DEC, 0);
    rage128_write(test, R128_DST_BRES_LNTH, 8);
    ...
    for (unsigned int x = 0; x < 8; x++) {
        g_assert_cmpuint(fb[3 * 32 + x], ==,
                         destination[x] == 0x55 ? 0xd1 : destination[x]);
    }

    /* Source-keyed packed HOST_DATA leaves key-color pixels untouched. */
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_CLR_CMP_CNTL_DIS |
    ...
    rage128_write(test, R128_DST_HEIGHT_WIDTH, (1U << 16) | 4U);
    rage128_write(test, R128_HOST_DATA_LAST, 0x22302210);
    qpci_memread(test->dev, test->framebuffer, 4 * 32, &fb[4 * 32], 4);
    g_assert_cmpuint(fb[4 * 32 + 0], ==, 0x10);
    g_assert_cmpuint(fb[4 * 32 + 1], ==, 0xcc);
    g_assert_cmpuint(fb[4 * 32 + 2], ==, 0x30);
    g_assert_cmpuint(fb[4 * 32 + 3], ==, 0xcc);

    rage128_test_stop(test);
}

static void test_rage128_host_color(void)
{
    Rage128Test *test = rage128_test_start();
    uint8_t fb[48 * 4];
    static const uint32_t stream24[] = {
        0x66112233, 0x88994455, 0xccbbaa77, 0xf6a1b2c3,
        0x0203d4e5, 0xffeedd01,
    };
    static const uint8_t expected24[][9] = {
        { 0x33, 0x22, 0x11, 0x66, 0x55, 0x44, 0x99, 0x88, 0x77 },
        { 0xc3, 0xb2, 0xa1, 0xf6, 0xe5, 0xd4, 0x03, 0x02, 0x01 },
    };

    /* 8-bpp rows are independently padded to a dword. */
    memset(fb, 0x5a, sizeof(fb));
    qpci_memwrite(test->dev, test->framebuffer, 0, fb, sizeof(fb));
    rage128_set_surface(test, 4); /* 32-byte stride */
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_BRUSH_NONE |
                  R128_GMC_DST_8BPP | R128_GMC_SRC_DATATYPE_COLOR |
                  R128_ROP3_SRCCOPY | R128_DP_SRC_SOURCE_HOST_DATA);
    rage128_write(test, R128_DST_X, 2);
    rage128_write(test, R128_DST_Y, 1);
    rage128_write(test, R128_DST_HEIGHT_WIDTH, (2U << 16) | 3U);
    rage128_write(test, R128_HOST_DATA0, 0xee121110);
    rage128_write(test, R128_HOST_DATA_LAST, 0xff222120);
    qpci_memread(test->dev, test->framebuffer, 0, fb, 32 * 4);
    for (unsigned int row = 0; row < 2; row++) {
        for (unsigned int x = 0; x < 3; x++) {
            g_assert_cmpuint(fb[(1 + row) * 32 + 2 + x], ==,
                             (row ? 0x20 : 0x10) + x);
        }
        g_assert_cmpuint(fb[(1 + row) * 32 + 1], ==, 0x5a);
        g_assert_cmpuint(fb[(1 + row) * 32 + 5], ==, 0x5a);
    }

    /* 16-bpp odd-width rows consume two padding bytes each. */
    memset(fb, 0x5a, sizeof(fb));
    qpci_memwrite(test->dev, test->framebuffer, 0, fb, sizeof(fb));
    rage128_set_surface(test, 2); /* 32-byte stride */
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_BRUSH_NONE |
                  R128_GMC_DST_16BPP | R128_GMC_SRC_DATATYPE_COLOR |
                  R128_ROP3_SRCCOPY | R128_DP_SRC_SOURCE_HOST_DATA);
    rage128_write(test, R128_DST_X, 1);
    rage128_write(test, R128_DST_Y, 0);
    rage128_write(test, R128_DST_HEIGHT_WIDTH, (2U << 16) | 3U);
    rage128_write(test, R128_HOST_DATA0, 0x33441122);
    rage128_write(test, R128_HOST_DATA0 + 4, 0xbbaa5566);
    rage128_write(test, R128_HOST_DATA0 + 8, 0x99aa7788);
    rage128_write(test, R128_HOST_DATA_LAST, 0xddccbbcc);
    qpci_memread(test->dev, test->framebuffer, 0, fb, 32 * 2);
    g_assert_cmpuint(fb[2], ==, 0x22);
    g_assert_cmpuint(fb[3], ==, 0x11);
    g_assert_cmpuint(fb[4], ==, 0x44);
    g_assert_cmpuint(fb[5], ==, 0x33);
    g_assert_cmpuint(fb[6], ==, 0x66);
    g_assert_cmpuint(fb[7], ==, 0x55);
    g_assert_cmpuint(fb[32 + 2], ==, 0x88);
    g_assert_cmpuint(fb[32 + 3], ==, 0x77);
    g_assert_cmpuint(fb[32 + 4], ==, 0xaa);
    g_assert_cmpuint(fb[32 + 5], ==, 0x99);
    g_assert_cmpuint(fb[32 + 6], ==, 0xcc);
    g_assert_cmpuint(fb[32 + 7], ==, 0xbb);
    g_assert_cmpuint(fb[8], ==, 0x5a);
    g_assert_cmpuint(fb[32 + 8], ==, 0x5a);

    /*
     * Three packed 24-bpp pixels take nine bytes, followed by three bytes
     * of row padding.  The first byte of row 1's second pixel lands at the
     * end of the first 16-byte HOST_DATA batch, so this also proves that a
     * pixel accumulator survives register flush boundaries.
     */
    memset(fb, 0x5a, sizeof(fb));
    qpci_memwrite(test->dev, test->framebuffer, 0, fb, sizeof(fb));
    rage128_set_surface(test, 2); /* 48-byte stride */
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_BRUSH_NONE |
                  R128_GMC_DST_24BPP | R128_GMC_SRC_DATATYPE_COLOR |
                  R128_ROP3_SRCCOPY | R128_DP_SRC_SOURCE_HOST_DATA);
    ...
    rage128_write(test, R128_DST_HEIGHT_WIDTH, (2U << 16) | 3U);
    for (unsigned int i = 0; i < 4; i++) {
        rage128_write(test, R128_HOST_DATA0 + i * 4, stream24[i]);
    }
    rage128_write(test, R128_HOST_DATA0, stream24[4]);
    rage128_write(test, R128_HOST_DATA_LAST, stream24[5]);
    qpci_memread(test->dev, test->framebuffer, 0, fb, sizeof(fb));
    for (unsigned int row = 0; row < 2; row++) {
        unsigned int offset = (1 + row) * 48 + 2 * 3;

        g_assert_cmpmem(&fb[offset], sizeof(expected24[row]),
                        expected24[row], sizeof(expected24[row]));
        for (unsigned int byte = 0; byte < 3; byte++) {
            g_assert_cmpuint(fb[offset - 3 + byte], ==, 0x5a);
            g_assert_cmpuint(fb[offset + 9 + byte], ==, 0x5a);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    if (test_arch_is_x86() && qtest_has_device("ati-vga")) {
        ...
        qtest_add_func("/display/pci/ati-vga/rage128-host-color",
                       test_rage128_host_color);
