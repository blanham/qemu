...
#define R128_GMC_CLR_CMP_CNTL_DIS           0x10000000
#define R128_GMC_BRUSH_8X8_MONO_FG_BG      0x00000000
#define R128_GMC_BRUSH_8X8_MONO_FG_LA      0x00000010
#define R128_GMC_BRUSH_32X1_MONO_FG_BG     0x00000060
#define R128_GMC_BRUSH_32X1_MONO_FG_LA     0x00000070
...
#define R128_ROP3_PATINVERT                  0x005a0000
...
#define R128_SRC_CMP_EQ_COLOR                0x00000004
#define R128_SRC_CMP_NEQ_COLOR               0x00000005
...
#define R128_PLL_WR_EN                       0x00000080
#define R128_PPLL_REF_DIV                    0x03
#define R128_PPLL_DIV_3                      0x07
#define R128_PLL_TEST_CNTL                   0x13
#define R128_PPLL_ATOMIC_UPDATE_W            0x00008000
...
static uint32_t rage128_pll_read(Rage128Test *test, uint8_t index)
{
    qpci_io_writeb(test->dev, test->mmio, R128_CLOCK_CNTL_INDEX,
                   index & 0x3f);
    return rage128_read(test, R128_CLOCK_CNTL_DATA);
}

static void rage128_pll_write(Rage128Test *test, uint8_t index,
                              uint32_t value)
{
    qpci_io_writeb(test->dev, test->mmio, R128_CLOCK_CNTL_INDEX,
                   (index & 0x3f) | R128_PLL_WR_EN);
    rage128_write(test, R128_CLOCK_CNTL_DATA, value);
}

static void test_rage128_pll_registers(void)
{
    Rage128Test *test = rage128_test_start_model(
        "rage128re", "", ATI_RAGE128_RE_DEVICE_ID);

    g_assert_cmphex(rage128_pll_read(test, R128_PPLL_REF_DIV), ==, 12);
    g_assert_cmphex(rage128_pll_read(test, R128_PPLL_DIV_3), ==,
                    56 | (4U << 16));

    rage128_pll_write(test, R128_PLL_TEST_CNTL, 0x11223344);
    qpci_io_writeb(test->dev, test->mmio, R128_CLOCK_CNTL_INDEX,
                   R128_PLL_TEST_CNTL | R128_PLL_WR_EN);
    qpci_io_writeb(test->dev, test->mmio, R128_CLOCK_CNTL_DATA + 3,
                   0xaa);
    g_assert_cmphex(rage128_pll_read(test, R128_PLL_TEST_CNTL), ==,
                    0xaa223344);

    /* CLOCK_CNTL_DATA is write-protected unless PLL_WR_EN is set. */
    qpci_io_writeb(test->dev, test->mmio, R128_CLOCK_CNTL_INDEX,
                   R128_PLL_TEST_CNTL);
    rage128_write(test, R128_CLOCK_CNTL_DATA, UINT32_MAX);
    g_assert_cmphex(rage128_pll_read(test, R128_PLL_TEST_CNTL), ==,
                    0xaa223344);

    /* Atomic update completes immediately in the synchronous model. */
    rage128_pll_write(test, R128_PPLL_REF_DIV,
                      R128_PPLL_ATOMIC_UPDATE_W | 0x41);
    g_assert_cmphex(rage128_pll_read(test, R128_PPLL_REF_DIV), ==, 0x41);

    rage128_test_stop(test);
}

static uint32_t rage128_load_le32(const uint8_t *bytes)
{
...
static void test_rage128_brushes(void)
{
    static const uint8_t rows[8] = {
        0xaa, 0x55, 0xf0, 0x0f, 0x81, 0x42, 0x24, 0x18,
    };
    Rage128Test *test = rage128_test_start();
    uint8_t fb[32 * 12];
    uint32_t brush_word0 = 0x0ff055aa;
    uint32_t brush_word1 = 0x18244281;

    memset(fb, 0x11, sizeof(fb));
    qpci_memwrite(test->dev, test->framebuffer, 0, fb, sizeof(fb));
    rage128_set_surface(test, 4);

    /* Opaque 8x8 brush: origin, MSB pixel order, and pattern ROP. */
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_BRUSH_8X8_MONO_FG_BG |
                  R128_GMC_DST_8BPP | R128_GMC_SRC_DATATYPE_COLOR |
                  R128_ROP3_PATINVERT);
    rage128_write(test, R128_DP_BRUSH_FRGD_CLR, 0xe0);
    rage128_write(test, R128_DP_BRUSH_BKGD_CLR, 0x20);
    rage128_write(test, R128_BRUSH_Y_X, (1U << 8) | 2U);
    rage128_write(test, R128_BRUSH_DATA0, brush_word0);
    rage128_write(test, R128_BRUSH_DATA0 + 4, brush_word1);
    g_assert_cmphex(rage128_read(test, R128_BRUSH_Y_X), ==,
                    (1U << 8) | 2U);
    g_assert_cmphex(rage128_read(test, R128_BRUSH_DATA0), ==,
                    brush_word0);
    g_assert_cmphex(rage128_read(test, R128_BRUSH_DATA0 + 4), ==,
                    brush_word1);
    rage128_write(test, R128_DST_X, 0);
    rage128_write(test, R128_DST_Y, 0);
    rage128_write(test, R128_DST_HEIGHT_WIDTH, (4U << 16) | 10U);

    qpci_memread(test->dev, test->framebuffer, 0, fb, sizeof(fb));
    for (unsigned int y = 0; y < 4; y++) {
        unsigned int py = (unsigned int)((int)y - 1) & 7U;

        for (unsigned int x = 0; x < 10; x++) {
            unsigned int px = (unsigned int)((int)x - 2) & 7U;
            uint8_t pattern = rows[py] & (UINT8_C(1) << (7U - px)) ?
                              0xe0 : 0x20;

            g_assert_cmpuint(fb[y * 32 + x], ==, pattern ^ 0x11);
        }
        g_assert_cmpuint(fb[y * 32 + 10], ==, 0x11);
    }

    /* Foreground-only 8x8 brush: LSB pixel order and transparency. */
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_BRUSH_8X8_MONO_FG_LA |
                  R128_GMC_DST_8BPP | R128_GMC_SRC_DATATYPE_COLOR |
                  R128_GMC_BYTE_LSB_TO_MSB | R128_ROP3_PATCOPY);
    rage128_write(test, R128_DP_BRUSH_FRGD_CLR, 0x7a);
    rage128_write(test, R128_BRUSH_Y_X, 5U << 8);
    rage128_write(test, R128_BRUSH_DATA0, 0x00000005);
    rage128_write(test, R128_DST_X, 0);
    rage128_write(test, R128_DST_Y, 5);
    rage128_write(test, R128_DST_HEIGHT_WIDTH, (1U << 16) | 8U);
    qpci_memread(test->dev, test->framebuffer, 5 * 32, &fb[5 * 32], 8);
    for (unsigned int x = 0; x < 8; x++) {
        g_assert_cmpuint(fb[5 * 32 + x], ==,
                         (x == 0 || x == 2) ? 0x7a : 0x11);
    }

    /* Opaque and transparent 32x1 brushes drive dashed line phase. */
    memset(fb, 0x44, sizeof(fb));
    qpci_memwrite(test->dev, test->framebuffer, 0, fb, sizeof(fb));
    rage128_set_surface(test, 4);
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_BRUSH_32X1_MONO_FG_BG |
                  R128_GMC_DST_8BPP | R128_GMC_SRC_DATATYPE_COLOR |
                  R128_GMC_BYTE_LSB_TO_MSB | R128_ROP3_PATCOPY);
    rage128_write(test, R128_DP_BRUSH_FRGD_CLR, 0xf0);
    rage128_write(test, R128_DP_BRUSH_BKGD_CLR, 0x0f);
    rage128_write(test, R128_BRUSH_Y_X, 3);
    rage128_write(test, R128_BRUSH_DATA0, 0x000000b2);
    rage128_write(test, R128_DP_CNTL_XDIR_YDIR_YMAJOR,
                  R128_LINE_X_LEFT_TO_RIGHT |
                  R128_LINE_Y_TOP_TO_BOTTOM);
    rage128_write(test, R128_DST_Y_X, (6U << 16) | 2U);
    rage128_write(test, R128_DST_BRES_ERR, UINT32_MAX);
    rage128_write(test, R128_DST_BRES_INC, 0);
    rage128_write(test, R128_DST_BRES_DEC, 0);
    rage128_write(test, R128_DST_BRES_LNTH, 10);

    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_BRUSH_32X1_MONO_FG_LA |
                  R128_GMC_DST_8BPP | R128_GMC_SRC_DATATYPE_COLOR |
                  R128_GMC_BYTE_LSB_TO_MSB | R128_ROP3_PATCOPY);
    rage128_write(test, R128_DP_BRUSH_FRGD_CLR, 0x99);
    rage128_write(test, R128_BRUSH_Y_X, 1);
    rage128_write(test, R128_BRUSH_DATA0, 0x0000000a);
    rage128_write(test, R128_DST_Y_X, (7U << 16) | 2U);
    rage128_write(test, R128_DST_BRES_ERR, UINT32_MAX);
    rage128_write(test, R128_DST_BRES_INC, 0);
    rage128_write(test, R128_DST_BRES_DEC, 0);
    rage128_write(test, R128_DST_BRES_LNTH, 8);
    ...
    for (unsigned int i = 0; i < 10; i++) {
        bool foreground = !!(UINT32_C(0xb2) &
                             (UINT32_C(1) << ((i + 3) & 31U)));

        g_assert_cmpuint(fb[6 * 32 + 2 + i], ==,
                         foreground ? 0xf0 : 0x0f);
    }
    for (unsigned int i = 0; i < 8; i++) {
        bool foreground = !!(UINT32_C(0x0a) &
                             (UINT32_C(1) << ((i + 1) & 31U)));

        g_assert_cmpuint(fb[7 * 32 + 2 + i], ==,
                         foreground ? 0x99 : 0x44);
    }
...
static void test_rage128_color_compare(void)
{
    static const uint8_t copy_source[8] = {
        0x10, 0x7e, 0x20, 0x7e, 0x30, 0x40, 0x7e, 0x50,
    };
    static const uint8_t dest_compare[8] = {
        0x33, 0x44, 0x33, 0x55, 0x66, 0x33, 0x77, 0x88,
    };
    static const uint8_t host_source[8] = {
        0x11, 0x7e, 0x22, 0x7e, 0x33, 0x44, 0x7e, 0x55,
    };
    Rage128Test *test = rage128_test_start();
    uint8_t fb[5 * 32];

    memset(fb, 0x55, sizeof(fb));
    memcpy(&fb[0 * 32], copy_source, sizeof(copy_source));
    memcpy(&fb[2 * 32], dest_compare, sizeof(dest_compare));
    memset(&fb[3 * 32], 0x66, 8);
    memset(&fb[4 * 32], 0x77, 8);
    qpci_memwrite(test->dev, test->framebuffer, 0, fb, sizeof(fb));
    rage128_set_surface(test, 4); /* 32-byte 8-bpp stride */

    /* All four comparison registers are ordinary readable 2D state. */
    rage128_write(test, R128_CLR_CMP_CLR_SRC, 0x11223344);
    rage128_write(test, R128_CLR_CMP_CLR_DST, 0x55667788);
    rage128_write(test, R128_CLR_CMP_MASK, 0x00ff00ff);
    rage128_write(test, R128_CLR_CMP_CNTL,
                  R128_SRC_CMP_NEQ_COLOR | R128_CLR_CMP_SRC_SOURCE);
    g_assert_cmphex(rage128_read(test, R128_CLR_CMP_CLR_SRC), ==,
                    0x11223344);
    g_assert_cmphex(rage128_read(test, R128_CLR_CMP_CLR_DST), ==,
                    0x55667788);
    g_assert_cmphex(rage128_read(test, R128_CLR_CMP_MASK), ==,
                    0x00ff00ff);
    g_assert_cmphex(rage128_read(test, R128_CLR_CMP_CNTL), ==,
                    R128_SRC_CMP_NEQ_COLOR | R128_CLR_CMP_SRC_SOURCE);

    /*
     * The historical driver includes CLR_CMP_CNTL_DIS in every baseline
     * master word, then programs compare state only for transparent ops.
     */
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_CLR_CMP_CNTL_DIS |
                  R128_GMC_BRUSH_NONE | R128_GMC_DST_8BPP |
                  R128_GMC_SRC_DATATYPE_COLOR | R128_ROP3_SRCCOPY |
                  R128_DP_SRC_SOURCE_MEMORY);
    g_assert_cmpuint(rage128_read(test, R128_CLR_CMP_CNTL), ==, 0);

    /* XAA transparent screen-to-screen copy: skip the source key. */
    rage128_write(test, R128_CLR_CMP_CLR_SRC, 0x7e);
    rage128_write(test, R128_CLR_CMP_MASK, 0xff);
    rage128_write(test, R128_CLR_CMP_CNTL,
                  R128_SRC_CMP_NEQ_COLOR | R128_CLR_CMP_SRC_SOURCE);
    rage128_write(test, R128_SRC_X, 0);
    rage128_write(test, R128_SRC_Y, 0);
    rage128_write(test, R128_DST_X, 0);
    rage128_write(test, R128_DST_Y, 1);
    rage128_write(test, R128_DST_HEIGHT_WIDTH, (1U << 16) | 8U);

    qpci_memread(test->dev, test->framebuffer, 1 * 32, &fb[1 * 32], 8);
    for (unsigned int x = 0; x < 8; x++) {
        g_assert_cmpuint(fb[1 * 32 + x], ==,
                         copy_source[x] == 0x7e ? 0x55 : copy_source[x]);
    }

    /*
     * Destination equality comparison gates a PATCOPY fill.  This also
     * proves that comparison cannot be bypassed by the pixman fill path.
     */
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_CLR_CMP_CNTL_DIS |
                  R128_GMC_BRUSH_SOLIDCOLOR | R128_GMC_DST_8BPP |
                  R128_GMC_SRC_DATATYPE_COLOR | R128_ROP3_PATCOPY);
    rage128_write(test, R128_DP_BRUSH_FRGD_CLR, 0xa6);
    rage128_write(test, R128_CLR_CMP_CLR_DST, 0x33);
    rage128_write(test, R128_CLR_CMP_MASK, 0xff);
    rage128_write(test, R128_CLR_CMP_CNTL, R128_SRC_CMP_EQ_COLOR);
    rage128_write(test, R128_DST_X, 0);
    rage128_write(test, R128_DST_Y, 2);
    rage128_write(test, R128_DST_HEIGHT_WIDTH, (1U << 16) | 8U);

    qpci_memread(test->dev, test->framebuffer, 2 * 32, &fb[2 * 32], 8);
    for (unsigned int x = 0; x < 8; x++) {
        g_assert_cmpuint(fb[2 * 32 + x], ==,
                         dest_compare[x] == 0x33 ? 0xa6 : dest_compare[x]);
    }

    /* XAA ImageWrite uses the same source-key comparison on HOST_DATA. */
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_CLR_CMP_CNTL_DIS |
                  R128_GMC_BRUSH_NONE | R128_GMC_DST_8BPP |
    ...
    rage128_write(test, R128_CLR_CMP_CNTL,
                  R128_SRC_CMP_NEQ_COLOR | R128_CLR_CMP_SRC_SOURCE);
    rage128_write(test, R128_DST_X, 0);
    rage128_write(test, R128_DST_Y, 3);
    rage128_write(test, R128_DST_HEIGHT_WIDTH, (1U << 16) | 8U);
    rage128_write(test, R128_HOST_DATA0,
                  rage128_load_le32(&host_source[0]));
    rage128_write(test, R128_HOST_DATA_LAST,
                  rage128_load_le32(&host_source[4]));

    qpci_memread(test->dev, test->framebuffer, 3 * 32, &fb[3 * 32], 8);
    for (unsigned int x = 0; x < 8; x++) {
        g_assert_cmpuint(fb[3 * 32 + x], ==,
                         host_source[x] == 0x7e ? 0x66 : host_source[x]);
    }

    /* A new baseline master word disables stale transparency state. */
    rage128_write(test, R128_DP_GUI_MASTER_CNTL,
                  R128_COMMON_MASTER | R128_GMC_CLR_CMP_CNTL_DIS |
                  R128_GMC_BRUSH_SOLIDCOLOR | R128_GMC_DST_8BPP |
                  R128_GMC_SRC_DATATYPE_COLOR | R128_ROP3_PATCOPY);
    ...
    rage128_write(test, R128_DP_BRUSH_FRGD_CLR, 0x99);
    rage128_write(test, R128_DST_X, 0);
    rage128_write(test, R128_DST_Y, 4);
    rage128_write(test, R128_DST_HEIGHT_WIDTH, (1U << 16) | 8U);

    qpci_memread(test->dev, test->framebuffer, 4 * 32, &fb[4 * 32], 8);
    for (unsigned int x = 0; x < 8; x++) {
        g_assert_cmpuint(fb[4 * 32 + x], ==, 0x99);
    }

    rage128_test_stop(test);
}

static void test_rage128_host_color_24(void)
{
    enum {
        WIDTH = 6,
        HEIGHT = 2,
    ...
    Rage128Test *test = rage128_test_start();
    uint8_t upload[HEIGHT * ROW_BYTES];
    uint8_t fb[HEIGHT * STRIDE];

    for (unsigned int row = 0; row < HEIGHT; row++) {
        for (unsigned int x = 0; x < WIDTH; x++) {
            unsigned int offset = row * ROW_BYTES + x * 3;

            upload[offset] = 0x10 + row * 0x20 + x;
            upload[offset + 1] = 0x50 + row * 0x20 + x;
            upload[offset + 2] = 0x90 + row * 0x20 + x;
        }
        upload[row * ROW_BYTES + 18] = 0xde;
        upload[row * ROW_BYTES + 19] = 0xad;
    }
    ...
    rage128_write(test, R128_DST_X, 1);
    rage128_write(test, R128_DST_Y, 0);
    rage128_write(test, R128_DST_HEIGHT_WIDTH,
                  (HEIGHT << 16) | WIDTH);

    for (unsigned int i = 0; i < 5; i++) {
        rage128_write(test, R128_HOST_DATA0 + (3 + i) * 4,
                      rage128_load_le32(&upload[i * 4]));
    }
    for (unsigned int i = 0; i < 5; i++) {
        rage128_write(test, R128_HOST_DATA0 + (4 + i) * 4,
                      rage128_load_le32(&upload[ROW_BYTES + i * 4]));
    }

    qpci_memread(test->dev, test->framebuffer, 0, fb, sizeof(fb));
    for (unsigned int row = 0; row < HEIGHT; row++) {
        for (unsigned int byte = 0; byte < 3; byte++) {
            g_assert_cmpuint(fb[row * STRIDE + byte], ==, 0xaa);
            g_assert_cmpuint(fb[row * STRIDE + (WIDTH + 1) * 3 + byte],
                             ==, 0xaa);
        }
        for (unsigned int x = 0; x < WIDTH; x++) {
            unsigned int source = row * ROW_BYTES + x * 3;
            unsigned int dest = row * STRIDE + (x + 1) * 3;

            g_assert_cmpmem(&fb[dest], 3, &upload[source], 3);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    if (test_arch_is_x86() && qtest_has_device("ati-vga")) {
        ...
        qtest_add_func("/display/pci/ati-vga/rage128-ddc",
                       test_rage128_ddc);
        qtest_add_func("/display/pci/ati-vga/rage128-pll",
                       test_rage128_pll_registers);
        ...
        qtest_add_func("/display/pci/ati-vga/rage128-lines",
                       test_rage128_lines);
        qtest_add_func("/display/pci/ati-vga/rage128-brushes",
                       test_rage128_brushes);
        qtest_add_func("/display/pci/ati-vga/rage128-2d",
                       test_rage128_2d);
        qtest_add_func("/display/pci/ati-vga/rage128-color-compare",
                       test_rage128_color_compare);
        qtest_add_func("/display/pci/ati-vga/rage128-host-color-24",
                       test_rage128_host_color_24);
    }
