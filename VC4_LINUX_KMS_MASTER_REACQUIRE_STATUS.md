...
Frontier: **`linux-vc4-kms-master-reacquire-visual-clear`**
...
- Active CRTC visible from new file: `True`
...
After the established inherited-file modeset and page-flip baseline, PID 1 drops DRM master on its original card file. A bounded child closes that inherited descriptor before reopening /dev/dri/card0, explicitly acquires master on the new drm_file, queues an event-driven page flip, consumes DRM_EVENT_FLIP_COMPLETE, and verifies the CRTC's framebuffer ID. The host freezes QEMU at the independent-file visual-ready marker and requires every captured XRGB8888 pixel to match the reacquisition pattern. The child then drops master and exits, after which the original drm_file must reacquire master before the render witness continues.
