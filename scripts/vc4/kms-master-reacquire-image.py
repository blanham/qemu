#!/usr/bin/env python3
"""Verify the XRGB8888 frame painted after VC4 DRM-master reacquisition."""
...
PATTERN_NAME = "vc4-native-kms-master-reacquire-xrgb8888-v1"
CHECKER_WIDTH = 40
CHECKER_HEIGHT = 24
DARK_GREEN = 0x24
BRIGHT_GREEN = 0xE0
...
def expected_pixel(
    ...
    checker = ((x // CHECKER_WIDTH) ^ (y // CHECKER_HEIGHT)) & 1
    red = 255 - x * 255 // (width - 1) if width > 1 else 0xFF
    green = BRIGHT_GREEN if checker else DARK_GREEN
    blue = y * 255 // (height - 1) if height > 1 else 0
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix="vc4-master-reacquire-image-"
    ) as temp_dir:
...
def main() -> int:
    ...
    if args.self_test:
        self_test()
        print("VC4 master-reacquisition image verifier self-test passed")
        return 0
