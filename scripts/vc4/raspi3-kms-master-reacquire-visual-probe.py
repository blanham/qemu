#!/usr/bin/env python3
"""Capture and verify a VC4 page flip after independent DRM-master reacquisition."""
...
VISUAL_READY_MARKER = "VC4_LINUX_KMS_MASTER_REACQUIRE_VISUAL_READY"
...
def main() -> int:
    ...
    image = load_sibling(
        "kms-master-reacquire-image.py", "vc4_master_reacquire_image"
    )
    ...
    image_record_path = out_dir / "master-reacquire-image.json"
    ...
    result: dict[str, Any] = {
        "schema_version": 1,
        "mode": "linux-direct-master-reacquire-visual",
    ...
    with tempfile.TemporaryDirectory(
        prefix="vc4-master-reacquire-visual-"
    ) as temp_s:
        temp = Path(temp_s)
        ...
        try:
            ...
            serial = base.read_serial(serial_path)
            result["serial_tail"] = serial.splitlines()[-700:]
            result["visual_ready_seen"] = VISUAL_READY_MARKER in serial
            result["required_completion_markers"] = {
                marker: marker in serial
                for marker in REQUIRED_COMPLETION_MARKERS
