#!/usr/bin/env python3
"""Exercise the exact discard-only vector48 delay used by bootcode.bin."""
...
EXACT_PC = 0x0000775E
RESULT_ADDR = 0x00047040

# Stock bootcode.bin at firmware commit 3d301dd924bcd758a4c8cb19fe8531031f033f43.
# Little-endian halfwords: f400 e038 0400.
EXACT_INSN = bytes.fromhex("00 f4 38 e0 00 04")

MARKER = 0x56343844  # "V48D"
R0_SENTINEL = 0x00102030
R12_SENTINEL = 0x12121212
R13_SENTINEL = 0x13131313
SR_SENTINEL = 0x0000000A
EXPECTED = (
    MARKER,
    R0_SENTINEL,
    R12_SENTINEL,
...
def build_firmware(path: Path, vc4: ModuleType) -> bytes:
    ...
    place(image, pc, branch, "branch to exact stock vector48 PC")

    place(image, EXACT_PC, EXACT_INSN, "exact stock vector48 delay")
    ...
    for reg, offset in ((1, 0), (0, 4), (12, 8), (13, 12), (30, 16)):
    ...
    if image[EXACT_PC:EXACT_PC + len(EXACT_INSN)] != EXACT_INSN:
        raise AssertionError("exact vector48 production bytes changed")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="vc4-vector48-delay-") as tmp_s:
        tmp = Path(tmp_s)
        firmware_path = tmp / "vector48-delay.bin"
        ...
        try:
            ...
            if loaded != EXACT_INSN:
                raise RuntimeError(
                    "exact stock vector48 fixture was not loaded: "
            ...
            if observed != EXPECTED:
                raise RuntimeError(
                    "vector48 delay result mismatch: "
            ...
            if "unimplemented opcode 0xf400" in diagnostics:
                raise RuntimeError(
                    "exact stock vector48 delay still reached illegal decode"
                )

            print(
                "VideoCore IV exact discard-only vector48 delay passed: "
                f"cpus={len(qom_types)} arm={arm_count} vc4={vc4_count} "
                f"stock-pc=0x{EXACT_PC:08x} "
                f"bytes={EXACT_INSN.hex()} "
                "decode=v16mov-discard-immediate "
                f"r0=0x{observed[1]:08x} "
                f"r12=0x{observed[2]:08x} "
                f"r13=0x{observed[3]:08x} "
                f"sr=0x{observed[4]:08x}"
            )
            qmp.execute("quit")
            proc.wait(timeout=5)
            return 0
