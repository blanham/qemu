...
out_dir=${1:-"$root_dir/build/vc4-linux-v3d-uapi-initramfs"}
...
for tool in "$cc" cpio gzip sha256sum strings; do
    if ! command -v "$tool" >/dev/null 2>&1; then
        printf 'required Linux VC4 DRM UAPI initramfs tool not found: %s\n' \
            "$tool" >&2
        exit 1
    fi
done
...
"$cc" \
    ...
    -Wl,--build-id=none \
    -o "$out_dir/root/init" \
    "$root_dir/tests/vc4/linux-v3d-uapi-init.c"
...
for marker in \
    ...
    VC4_LINUX_DRM_GEM_MEMORY_OK \
    VC4_LINUX_DRM_UAPI_OK \
    VC4_LINUX_DRM_PROBE_DONE \
...
printf 'Built VC4 Linux DRM UAPI initramfs at %s\n' "$out_dir"
