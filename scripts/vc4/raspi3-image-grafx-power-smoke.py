...
IMAGE_STAGE1 = PM_ENAB | PM_INRUSH_20_MA | PM_POWUP
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="vc4-raspi3-image-power-") as tmp_s:
        ...
        try:
            ...
            if readl(qtest, power, PM_IMAGE_ARM) != PM_ENAB:
                raise RuntimeError("PM_IMAGE reset value is not 0x00001000")
            if readl(qtest, power, PM_GRAFX_ARM) != PM_ENAB:
                raise RuntimeError("PM_GRAFX reset value is not 0x00001000")
            ...
            # Powering down clears read-only completion bits when POWUP is
            # removed while retaining firmware-selected reset/config fields.
