...
static uint32_t ati_load_pixel(const ATI2DCtx *ctx, const uint8_t *ptr)
{
    unsigned int bypp = ctx->bpp / BITS_PER_BYTE;
    uint32_t value = 0;

    for (unsigned int byte = 0; byte < bypp; byte++) {
        unsigned int shift = ctx->vga->big_endian_fb ?
                             (bypp - 1 - byte) * BITS_PER_BYTE :
                             byte * BITS_PER_BYTE;

        value |= (uint32_t)ptr[byte] << shift;
    }
    return value;
}
...
static bool ati_color_compare_supported(const ATI2DCtx *ctx)
{
    ...
    return function == 0 || function == SRC_CMP_EQ_COLOR ||
                                   ...
                                   const uint8_t *dst)
{
    ...
    mask = ctx->clr_cmp_mask & ati_pixel_mask(ctx->bpp / BITS_PER_BYTE);
    if (ctx->clr_cmp_cntl & CLR_CMP_SRC_SOURCE) {
        value = src ? ati_load_pixel(ctx, src) : ctx->src_clr;
        reference = ctx->clr_cmp_clr_src;
    } else {
        value = ati_load_pixel(ctx, dst);
        reference = ctx->clr_cmp_clr_dst;
    }
    equal = ((value ^ reference) & mask) == 0;

    if (function == SRC_CMP_EQ_COLOR) {
        return equal;
    }
    if (function == SRC_CMP_NEQ_COLOR) {
        return !equal;
    }
    return false;
}
                                ...
                                const QemuRect *vis_dst)
{
    ...
    for (int row = 0; row < vis_dst->height; row++) {
        ...
        for (int col = 0; col < vis_dst->width; col++) {
            ...
            if (needs_src && ati_source_is_memory(ctx)) {
                if (ctx->src_valid &&
            ...
            if (!ati_color_compare_pass(ctx, src, dst)) {
                continue;
            }
...
static bool ati_2d_do_blt(const ATI2DCtx *ctx, uint8_t use_pixman)
{
    ...
    if (!ati_color_compare_supported(ctx)) {
        qemu_log_mask(LOG_UNIMP,
                      "ATI 2D color comparison function %x is not implemented\n",
                      ctx->clr_cmp_cntl & CLR_CMP_FN_MASK);
        return false;
    }
    ...
    if ((use_pixman & BIT(1)) && !ati_color_compare_enabled(ctx) &&
        ctx->rop3 == 0xcc &&
    ...
    if ((use_pixman & BIT(0)) && !ati_color_compare_enabled(ctx) &&
        ctx->bpp != 24 &&
        (ctx->write_mask & pixel_mask) == pixel_mask &&
...
void ati_2d_line(ATIVGAState *s)
{
    ...
    if (!ati_color_compare_supported(&ctx)) {
        qemu_log_mask(LOG_UNIMP,
                      "ATI 2D line color comparison function %x is not implemented\n",
                      ctx.clr_cmp_cntl & CLR_CMP_FN_MASK);
        return;
    }
    if (ati_color_compare_uses_source(&ctx)) {
        qemu_log_mask(LOG_UNIMP,
                      "Source color comparison on ATI 2D lines is not implemented\n");
        return;
    }
    ...
    for (uint32_t i = 0; i < length; i++) {
        if (ctx.scissor.width > 0 && ctx.scissor.height > 0 &&
            ...
            if (draw && offset + bypp <= ctx.dst_size &&
                ati_color_compare_pass(&ctx, NULL,
                                       ctx.dst_bits + offset)) {
                uint8_t *dst = ctx.dst_bits + offset;

                for (unsigned int byte = 0; byte < bypp; byte++) {
                    uint8_t pattern = ati_reg_pixel_byte(&ctx, pixel_pattern,
                                                         byte);
                    uint8_t mask = ati_reg_pixel_byte(&ctx, ctx.write_mask,
                                                      byte);
...
void ati_2d_blt(ATIVGAState *s)
{
    ...
    if (src_source == DP_SRC_HOST) {
        /* Begin a HOST_DATA blit with a fresh stream parser. */
...
static uint8_t ati_host_data_byte(const ATIHostDataState *host,
                                  unsigned int index)
{
    return host->acc[index / sizeof(uint32_t)] >>
           ((index % sizeof(uint32_t)) * BITS_PER_BYTE);
}

static unsigned int ati_host_data_expand_color(ATIVGAState *s,
                                                const ATI2DCtx *ctx,
                                                uint8_t *pix_buf,
                                                ...
                                                unsigned int dst_height)
{
    ...
    for (unsigned int i = 0; i < input_bytes && host->active; i++) {
        uint8_t value = ati_host_data_byte(host, i);
        ...
        host->pixel_accumulator |=
            (uint32_t)value << (host->pixel_bytes_used * BITS_PER_BYTE);
        ...
        for (unsigned int byte = 0; byte < bypp; byte++) {
            pix_buf[pix_count * bypp + byte] =
                host->pixel_accumulator >> (byte * BITS_PER_BYTE);
        }
        pix_count++;
        ...
        if (host->col >= dst_width) {
            ...
            if (host->row >= dst_height) {
                host->active = false;
                host->padding_remaining = 0;
            } else {
                unsigned int row_bytes = dst_width * bypp;

                host->padding_remaining =
                    (sizeof(uint32_t) -
                     (row_bytes & (sizeof(uint32_t) - 1))) &
                    (sizeof(uint32_t) - 1);
            }
        }
    }
...
static unsigned int ati_host_data_expand_mono(ATIVGAState *s,
                                               const ATI2DCtx *ctx,
                                               uint8_t *pix_buf,
                                               uint8_t *valid_buf,
                                               unsigned int input_bytes,
                                               ...
                                               uint32_t src_datatype)
{
    ...
    unsigned int bypp = ctx->bpp / BITS_PER_BYTE;
    unsigned int pix_count = 0;
    bool lsb_first = s->regs.dp_datatype & DP_BYTE_PIX_ORDER;

    for (unsigned int i = 0; i < input_bytes && host->active; i++) {
        uint8_t value = ati_host_data_byte(host, i);

        for (unsigned int bit = 0; bit < BITS_PER_BYTE && host->active; bit++) {
            bool is_fg;
            uint32_t color;
            ...
            is_fg = value & BIT(lsb_first ? bit : BITS_PER_BYTE - 1 - bit);
            color = is_fg ? s->regs.dp_src_frgd_clr :
                            s->regs.dp_src_bkgd_clr;
            ati_store_reg_pixel(ctx, &pix_buf[pix_count * bypp], color);
            valid_buf[pix_count] =
...
bool ati_host_data_flush(ATIVGAState *s)
{
    ...
    input_bytes = MIN(s->host_data.next,
                      (uint32_t)ARRAY_SIZE(s->host_data.acc)) *
                  sizeof(uint32_t);
    if (!input_bytes) {
        return s->host_data.active;
    }

    bypp = ctx.bpp / BITS_PER_BYTE;
    row = s->host_data.row;
    col = s->host_data.col;
    ...
    if (src_datatype == SRC_COLOR) {
        pix_count = ati_host_data_expand_color(s, &ctx, pix_buf,
                                               input_bytes, dst_width,
                                               dst_height);
    } else {
        pix_count = ati_host_data_expand_mono(s, &ctx, pix_buf, valid_buf,
                                              input_bytes, dst_width,
                                              dst_height, src_datatype);
    }
    ...
    /* Blit one scanline chunk at a time. Padding never enters pix_buf. */
    idx = 0;
