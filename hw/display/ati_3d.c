                                        ...
                                        const ATIVGAState *s)
{
    ...
    }
    {
        unsigned int lod_bias =
            (tex_control & ATI_3D_TEX_LOD_BIAS_MASK) >>
            ATI_3D_TEX_LOD_BIAS_SHIFT;

        if (lod_bias != 0 && lod_bias != ATI_3D_TEX_LOD_BIAS_ZERO) {
            qemu_log_mask(LOG_UNIMP,
                          "ATI Rage 128 nonzero texture LOD bias is not implemented\n");
            return false;
        }
    }
                            ...
                            const float src[4], float z)
{
    ...
    if (ctx->tex_control & ATI_3D_ALPHA_ENABLE) {
        ...
        color_pixel = ati_3d_pack_surface_color(ctx->color_surface, src);
    }
                                ...
                                unsigned int *stride)
{
    ...
    *stride = ati_3d_vertex_stride(format);
    ...
    if (tex_control & (ATI_3D_FOG_ENABLE |
        ...
        qemu_log_mask(LOG_UNIMP,
                      "ATI Rage 128 fogged or stencil 3D is not implemented\n");
        return false;
    }
