...
static uint64_t ati_mm_read(void *opaque, hwaddr addr, unsigned int size)
{
    ...
    if (ati_pm4_mm_read(s, addr, size, &val)) {
    ...
    case CLOCK_CNTL_DATA ... CLOCK_CNTL_DATA + 3:
        val = ati_reg_read_offs(
            s->pll_regs[s->regs.clock_cntl_index & PLL_INDEX_MASK],
            addr - CLOCK_CNTL_DATA, size);
        break;
                           ...
                           uint64_t data, unsigned int size)
{
    ...
    if (ati_pm4_mm_write(s, addr, data, size)) {
        return;
    }
    switch (addr) {
    case MM_INDEX:
    ...
    case CLOCK_CNTL_INDEX ... CLOCK_CNTL_INDEX + 3:
        ...
        s->regs.clock_cntl_index &=
            PLL_INDEX_MASK | PLL_WR_EN | PLL_DIV_SEL;
        break;
    case CLOCK_CNTL_DATA ... CLOCK_CNTL_DATA + 3:
        if (s->regs.clock_cntl_index & PLL_WR_EN) {
            unsigned int index =
                s->regs.clock_cntl_index & PLL_INDEX_MASK;
            uint32_t value = s->pll_regs[index];

            ati_reg_write_offs(&value, addr - CLOCK_CNTL_DATA,
                               data, size);
            /* The software engine latches PLL updates synchronously. */
            if (index == PPLL_REF_DIV &&
                (value & PPLL_ATOMIC_UPDATE_W)) {
                value &= ~PPLL_ATOMIC_UPDATE_W;
            }
            s->pll_regs[index] = value;
        }
        break;
    case BIOS_0_SCRATCH ... BUS_CNTL - 1:
    {
...
{
    ati_mm_write(s, addr, data, sizeof(data));
}
...
static void ati_vga_reset(DeviceState *dev)
{
    ...
    s->pll_regs[PPLL_REF_DIV] = 12;
    s->pll_regs[PPLL_DIV_3] = 56 | (4U << 16);
    ati_vga_reset_2d(s);
    ati_pm4_reset(s, true);
    if (!s->rage128_pci &&
        s->dev_id == PCI_DEVICE_ID_ATI_RAGE128_PF) {
