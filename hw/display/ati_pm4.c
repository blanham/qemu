                            ...
                            const uint32_t *payload, unsigned int count)
{
    ...
    case R128_PM4_CNTL_LOAD_PALETTE:
        if (count < 1 ||
            !ati_3d_load_palette(s, payload[0], &payload[1], count - 1)) {
            ati_pm4_fault(s, "invalid LOAD_PALETTE packet");
            return false;
        }
        return true;
    ...
    case R128_PM4_3D_RNDR_GEN_INDX_PRIM:
        if (count < 4) {
            ati_pm4_fault(s, "short indexed 3D primitive packet");
            return false;
        }
        if (!ati_3d_draw_indexed(s, payload[0], payload[1],
                                 ...
                                 count > 4 ? count - 4 : 0)) {
            ati_pm4_fault(s, "indexed 3D primitive command failed");
            return false;
        }
        return true;
    ...
    case R128_PM4_3D_RNDR_GEN_PRIM:
        if (count < 2) {
            ati_pm4_fault(s, "short inline 3D primitive packet");
            return false;
        }
        if (!ati_3d_draw_inline(s, payload[0], payload[1],
                                count > 2 ? &payload[2] : NULL,
                                count > 2 ? count - 2 : 0)) {
            ati_pm4_fault(s, "inline 3D primitive command failed");
            return false;
        }
        return true;
